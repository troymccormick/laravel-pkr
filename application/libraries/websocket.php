...
class Websocket
{
	...
	*/
	...
	function wsStartServer($host, $port) {
		...
		//Console::write('** Server Started @ ' . date('Y-m-d H:i:s'), 'white');
		//Console::write('** Server is running and listening on port ' . $port, 'white');
	...
	function wsAddClient($socket, $clientIP) {
		...
		$this->wsClients[$clientID] = array($socket, '', self::WS_READY_STATE_CONNECTING, time(), false, 0, $clientIP, false, 0, '', 0, 0, $player);
